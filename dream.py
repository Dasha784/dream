...
def build_interpret_prompt(struct_json: str, mode: str, lang: str) -> str:
    ...
    if lang == "ru":
        example = (
            ...
            "PSYCH: МИНИМУМ 10 предложений (желательно 12–18) ЖИВОГО анализа с ДЕТАЛЬНЫМ РАЗБОРОМ КАЖДОГО ЭЛЕМЕНТА.\n"
            "ЗАПРЕЩЕНО писать короткие общие фразы типа 'Символический сон про свет, небо. Этот сон содержит важные послания.'\n"
            "ЗАПРЕЩЕНО обобщать без разбора элементов типа 'Этот сон содержит важные послания о внутреннем состоянии.'\n"
            "ОБЯЗАТЕЛЬНО:\n"
            "  1) Разбери КАЖДЫЙ ключевой элемент сна ОТДЕЛЬНО в отдельном предложении/абзаце (лестница, падение, океан, свет, небо, ключи, двери и т.д.)\n"
            "  2) Для КАЖДОГО элемента объясни что КОНКРЕТНО означает этот элемент психологически, используя значения из СПРАВОЧНИКА СИМВОЛИКИ.\n"
            "     Формат: 'Элемент 🪜 — конкретное значение. Развернутое объяснение в контексте этого сна. Как это работает психологически.'\n"
            "     Например: 'Свет ✨ — согласно справочнику символики, это надежда, понимание, знание. В твоем сне свет означает...'\n"
            "  3) Покажи связь между элементами — как они работают вместе, что получается в итоге\n"
            "  4) Раскрой СМЫСЛ всего сна — что он значит в реальной жизни, какие внутренние процессы отражает, какие скрытые послания несёт\n"
