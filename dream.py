...
def build_interpret_prompt(struct_json: str, mode: str, lang: str) -> str:
    ...
    if lang == "ru":
        example = (
            ...
            "PSYCH: 5–10 предложений ЖИВОГО анализа. ОБЯЗАТЕЛЬНО: раскрой СМЫСЛ сна, что он значит в реальной жизни, какие скрытые послания несёт, какие внутренние процессы отражает. Создай из сна целый мир, сделай интересно читать. Используй конкретные детали из структуры (места, действия, персонажи, символы). Объясни, как символы связаны с жизнью человека.\n"
        ...
        example = (
            ...
            "PSYCH: 5–10 речень ЖИВОГО аналізу. ОБОВ'ЯЗКОВО: розкрий СМИСЛ сну, що він означає в реальному житті, які приховані послання несе, які внутрішні процеси відображає. Створи зі сну цілий світ, зроби цікаво читати. Використовуй конкретні деталі зі структури (місця, дії, персонажі, символи). Поясни, як символи пов'язані з життям людини.\n"
        ...
        example = (
            ...
            "PSYCH: 5–10 sentences of LIVING analysis. MUST: uncover the MEANING of the dream, what it means in real life, what hidden messages it carries, what inner processes it reflects. Create a whole world from the dream, make it interesting to read. Use concrete details from structure (places, actions, characters, symbols). Explain how symbols relate to the person's life.\n"
